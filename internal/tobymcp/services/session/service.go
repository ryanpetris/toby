...
// Resources returns session introspection MCP resources.
	...
	return []tobymcp.Resource{
		{
			...
			Description: "Guidance for agent-managed MCP resources and automatic lifecycle behavior.",
			FS:          resourceDocs,
		...
		},
		{
			...
			Text: func(ctx context.Context, session *tobymcp.Session) (string, error) {
		...
		},
		{
			...
			Description: "Active and available Toby tools plus models endpoint summaries for this session.",
			Text: func(ctx context.Context, session *tobymcp.Session) (string, error) {
