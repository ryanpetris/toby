...
// Exercises per-tool launch flag parsing and command passthrough boundaries.
...
func TestParseLaunchCommandUsesCobraFlagsAndPassthrough(t *testing.T) {
	...
	parsed, err := executeTestLaunchParser(t, []string{"proj", "--debug", "--yolo", "--with-gh", "--upgrade", "--image=node:test", "--pull=never", "--", "foo", "--", "bar"}, ctxTools)
	if err != nil {
		t.Fatal(err)
	}
	...
	if parsed.Overrides.Pull != image.PullNever {
		t.Fatalf("pull = %q", parsed.Overrides.Pull)
	}
...
func TestParseLaunchCommandRejectsInvalidPullPolicy(t *testing.T) {
	_, err := executeTestLaunchParser(t, []string{"proj", "--pull=sometimes"}, nil)
	if err == nil || !strings.Contains(err.Error(), "--pull must be one of") {
		t.Fatalf("error = %v", err)
	}
}

func TestParseLaunchCommandSelectsQuietMode(t *testing.T) {
	parsed, err := executeTestLaunchParser(
		t,
		[]string{"proj", "--quiet"},
		nil,
	)
	...
	if !parsed.Options.Quiet {
		t.Fatal("quiet launch flag was not preserved")
	}
}

func TestParseLaunchCommandRejectsDebugWithQuiet(t *testing.T) {
	_, err := executeTestLaunchParser(
		t,
		[]string{"proj", "--debug", "--quiet"},
		nil,
	)
	if err == nil ||
		!strings.Contains(
