...
// The per-tool launch commands: sandbox and companion-tool flags plus the
// argument handling that turns a `toby <tool> -- ...` invocation into a launch.
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if effectiveConfigPath != "" {
				project, err := launchconfig.ResolveDirectLaunchProject(
					params.Paths,
					parsed.Options,
					params.TobyConfig.Settings().
						AllowExternalProjectsEnabled(),
				)
				if err != nil {
					return err
				}
...
func addSandboxFlags(cmd *cobra.Command) {
	cmd.Flags().String("project", "", "Project directory to mount and chdir into. Relative paths resolve from $XDG_PROJECTS_DIR; defaults to <env>.")
	cmd.Flags().String("image", "", "OCI image to use for the sandbox.")
	cmd.Flags().String("pull", "", "OCI image pull policy: if-missing, always, or never.")
}
