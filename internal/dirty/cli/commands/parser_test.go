...
func TestParseLaunchCommandUsesCobraFlagsAndPassthrough(t *testing.T) {
	ctxTools := []tools.Tool{
		contextTool{Base: tools.Base{Metadata: tools.Metadata{Name: tools.NpmToolName, LaunchHelp: "Launch Node Package Manager"}}},
		contextTool{Base: tools.Base{Metadata: tools.Metadata{Name: tools.GitHubCliToolName, CLIName: "gh", LaunchHelp: "Launch GitHub CLI"}}},
	}
	...
	if got, want := parsed.RequestedTools, []string{tools.GitHubCliToolName, tools.OpenCodeToolName}; !reflect.DeepEqual(got, want) {
...
func executeTestLaunchParser(t *testing.T, args []string, contextTools []tools.Tool) (launchconfig.DirectLaunch, error) {
	t.Helper()
	primary := contextTool{Base: tools.Base{Metadata: tools.Metadata{Name: tools.OpenCodeToolName, LaunchHelp: "Launch OpenCode"}}}
