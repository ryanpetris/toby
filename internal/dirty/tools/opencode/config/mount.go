...
func NewRenderer(registry *providers.Registry) (*Renderer, error) {
	if registry == nil {
		return nil, errors.New("opencode renderer requires a provider registry")
	}
	return &Renderer{providers: registry}, nil
}
...
func (r *Renderer) newMount(controlHost, tobyMCPURL string, instructions []string, cfg *tobyconfig.Service, proxy *httpproxy.Service, mcpProxy *mcpproxy.Service) (*Mount, error) {
	...
	return &Mount{controlHost: controlHost, tobyMCPURL: tobyMCPURL, instructions: append([]string(nil), instructions...), tobyConfig: cfg, proxy: proxy, mcpProxy: mcpProxy, providers: r.providers}, nil
}
...
func (m *Mount) tobySyntheticConfig(ctx context.Context) (map[string]any, error) {
	...
	for name, provider := range m.tobyConfig.Providers() {
		...
		if converted != nil {
			providerConfigs[name] = converted
		}
	}
...
func (m *Mount) syntheticProvider(ctx context.Context, providerID string, provider tobyconfig.ProviderConfig) (map[string]any, error) {
	...
	proxyID, err := m.proxy.RegisterUpstream(provider.BaseURL, headers)
	if err != nil {
...
func (m *Mount) fetchProviderModels(ctx context.Context, providerID string, provider tobyconfig.ProviderConfig) (map[string]any, error) {
	...
	models, err := m.providers.LookupModels(ctx, kind, provider.BaseURL, headerStrings(headers))
	if err != nil {
		return nil, err
	}
