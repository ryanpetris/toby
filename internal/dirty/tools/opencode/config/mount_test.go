...
func TestNewRendererRequiresRegistry(t *testing.T) {
	if _, err := NewRenderer(nil); err == nil {
		t.Fatal("expected nil provider registry to fail")
	}
}
...
func contextFilesWithTobyConfig(t *testing.T, client *http.Client, projectRoot string, instructions []string, cfg *tobyconfig.Service) ([]contextfiles.File, []error, error) {
	...
	recorder := tooltest.NewSandbox("")
	service.SetSandbox(recorder)
	warnings, err := renderer.RegisterContextFiles(context.Background(), service.Registrar(context.Background()), testControlHost, testTobyMCPURL, instructions, cfg, proxy, nil)
	if err != nil {
		return nil, warnings, err
	}
	return recorder.Files, warnings, nil
}
