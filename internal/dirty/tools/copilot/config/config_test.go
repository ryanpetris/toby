...
`))
	...
	for _, want := range []string{`"docs"`, `"tools": [`, `"remote"`, `"type": "http"`, `"url": "http://127.0.0.1:12345/proxy/`} {
...
func renderContextFiles(t *testing.T, instructions [][]byte, cfg *tobyconfig.Service) ([]contextfiles.File, error) {
	...
	if err := RegisterContextFiles(service.Registrar(t.Context()), instructions, cfg, "127.0.0.1:12345", testTobyMCPURL, proxy, mcpProxy); err != nil {
		return nil, err
	}
	return recorder.Files, nil
}
