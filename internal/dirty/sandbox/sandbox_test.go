...
func (e testRuntime) Name() string { return e.name }

func (e testRuntime) Priority() int { return e.priority }

func (e testRuntime) Available() error { return nil }

func (e testRuntime) NewInstance(spec Spec) (Instance, error) {
...
func TestProjectUnderProjectRootAccepted(t *testing.T) {
	...
	if !ok || visible != filepath.Join(layout.Workspace, "demo") {
...
func TestBaseInstancePathAndEndpointHelpers(t *testing.T) {
	...
	instance.workdir = ""
	if got := instance.ChdirDir(); got != filepath.Join(layout.Workspace, "app") {
...
func testFactory(paths config.Paths) Factory {
	factory, err := NewFactory(paths, []Runtime{
		testRuntime{name: RuntimeDocker, paths: paths},
		testRuntime{name: "alt", priority: 1, paths: paths},
	})
	if err != nil {
		panic(err)
	}
