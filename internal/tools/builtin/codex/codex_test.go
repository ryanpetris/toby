...
func TestInstallAllowsCodexPackageScripts(t *testing.T) {
	cdx, sandbox, _ := newTestCodex(t, testConfig(t, false))
	var got []string
	...
	if err := cdx.Install(t.Context(), false); err != nil {
		t.Fatal(err)
	}
	want := []string{"npm", "install", "-g", "--allow-scripts=@openai/codex", "@openai/codex"}
