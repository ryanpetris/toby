...
// Tests Cursor's native generated files.
...
func TestNativeFilesUseExactPathsAndMCPTransports(t *testing.T) {
	files, err := NativeFiles("cursor", toolfiles.Ownership{UID: 1000, GID: 1001}, sessionconfig.Config{
	...
	if instructionFile.Mode != 0o644 || instructionFile.UID != 1000 || instructionFile.GID != 1001 {
		t.Fatalf("native instructions metadata = %#v", instructionFile)
	}
	got := string(instructionFile.Data)
	if !strings.HasPrefix(got, "---\nalwaysApply: true\n---\n") {
		t.Fatalf("native instructions missing Cursor rule frontmatter: %q", got)
	}
	if !strings.Contains(got, "# native instructions\n") {
		t.Fatalf("native instructions missing body: %q", got)
	}
}
