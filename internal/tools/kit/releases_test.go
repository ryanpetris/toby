...
// Tests bounded GitHub release metadata requests.
...
func TestGetJSONSetsHeadersAndDecodesResponse(t *testing.T) {
	...
	if err := GetJSON(context.Background(), client, nil, "https://example.invalid/release", "application/json", &got); err != nil {
		t.Fatal(err)
	}
	if got.Name != "toby" {
...
func TestGetJSONReportsNon2xxBody(t *testing.T) {
	...
	err := GetJSON(context.Background(), client, nil, "https://example.invalid/release", "", &got)
