...
func TestProvideMetadataAndLaunch(t *testing.T) {
	...
	if svc.Name() != tool.GitHubCliToolName || svc.CommandName() != "gh" || svc.LaunchHelp() == "" {
		t.Fatalf("metadata = name %q command %q help %q", svc.Name(), svc.CommandName(), svc.LaunchHelp())
	}
	var got []string
	run := &tool.RunContext{Extra: []string{"repo", "view"}, Launch: func(_ context.Context, argv []string, _ tool.ExecOptions) (int, error) {
...
func TestRegisterContextFilesWritesInstaller(t *testing.T) {
	...
	if len(files) != 1 || files[0].Path != githubCLIInstallPath || files[0].Mode != 0o500 {
