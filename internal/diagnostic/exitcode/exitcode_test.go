...
// Tests explicit process exit-code errors and extraction.
...
func TestIsSilent(t *testing.T) {
	if !IsSilent(Code(7)) {
		t.Fatal("Code should be silent")
	}
	if IsSilent(New(7, "message")) {
		t.Fatal("New should not be silent")
	}
	if IsSilent(errors.New("plain")) {
		t.Fatal("plain error should not be silent")
	}
}
