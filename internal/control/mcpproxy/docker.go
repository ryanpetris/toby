...
func NewDockerRunner(containers *manager.Service) *DockerRunner {
	return &DockerRunner{containers: containers, httpRun: map[string]testcontainers.Container{}}
}

func NewDockerRuntime(containers *manager.Service) RuntimeResult {
...
func (r *DockerRunner) meta(spec SidecarSpec) manager.Meta {
	return manager.Meta{
		Label: spec.Name,
		Kind:  manager.KindMCPSidecar,
