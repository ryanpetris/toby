...
func runGit(ctx context.Context, repository, repoPath string, args []string) control.GitResult {
	...
	cmd.Stderr = &stderr
	err := cmd.Run()
	result := control.GitResult{Repository: repository, Stdout: stdout.String(), Stderr: stderr.String()}
	if err == nil {
		return result
	}
