...
const FxServiceGroup = "toby.sandbox.manager.services"
...
type RegistryParams struct {
	...
	Services []Service `group:"toby.sandbox.manager.services"`
}
...
func NewRegistry(params RegistryParams) (*Registry, error) {
	...
	for _, service := range params.Services {
		...
		for _, command := range service.Commands() {
			...
			if method == "" {
				return nil, fmt.Errorf("sandbox manager command must define a method")
			}
			if _, exists := registry.commands[method]; exists {
				return nil, fmt.Errorf("duplicate sandbox manager command: %s", method)
			}
...
func (r *Registry) Handle(ctx context.Context, runtime *Runtime, req control.RPCRequest) ([]byte, error) {
	if r == nil {
		return control.ResponseError(req.ID, control.CodeInternalError, "sandbox manager command registry is not configured", nil), syscall.ENOSYS
	}
