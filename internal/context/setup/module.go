...
// hooks.
func NewLifecycleHooks(cfg *appconfig.Service, contextFiles *contextfiles.Service) HooksResult {
	return HooksResult{
		AgentInstructions: lifecycle.Hook{
			...
			Run: func(ctx context.Context, _ lifecycle.Context) error {
				_, err := contextFiles.AddInstructionFS(ctx, TobyAgentsPath, AgentFiles(), tobyAgentsFile, 0o644)
				return err
			},
		},
