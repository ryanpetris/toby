...
// Tests internal Git error and process-status translation.
...
func TestGitErrorMappings(t *testing.T) {
	...
	if !errors.Is(errnoFor(notVisible), syscall.EACCES) || rpcErrorCode(notVisible) != hostaction.CodeProjectNotVisible {
		t.Fatalf("not visible mappings = %v, %d", errnoFor(notVisible), rpcErrorCode(notVisible))
	}
	if !errors.Is(errnoFor(syscall.EINVAL), syscall.EINVAL) || rpcErrorCode(syscall.EINVAL) != hostaction.CodeInvalidParams {
		t.Fatalf("EINVAL mappings = %v, %d", errnoFor(syscall.EINVAL), rpcErrorCode(syscall.EINVAL))
	}
	if rpcErrorCode(syscall.ENOSYS) != hostaction.CodeInternalError {
		t.Fatalf("ENOSYS rpc code = %d", rpcErrorCode(syscall.ENOSYS))
	}
	if !errors.Is(errnoFor(errors.New("other")), syscall.EIO) || rpcErrorCode(errors.New("other")) != hostaction.CodeInternalError {
