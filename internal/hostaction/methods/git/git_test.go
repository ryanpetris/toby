...
// Exercises host Git operations, repository capability retention, and RPC
// result behavior.
...
type directTestRunner struct{}

var _ CommandRunner = directTestRunner{}

func (directTestRunner) RunHostCommand(
	ctx context.Context,
	directory *os.File,
	command []string,
	stdin *os.File,
	...
	args := append(
		[]string{"-C", fmt.Sprintf("/proc/self/fd/%d", directoryFD)},
		command[1:]...,
	)
...
func (m *fakeResolver) OpenVisibleHostDirectory(
	repository string,
) (*os.File, error) {
	if m.visibleErr != nil {
		return nil, m.visibleErr
	}
	...
	return nil, errors.New("repository is not visible")
}
...
func handleGit(t *testing.T, resolver *fakeResolver, request []byte) ([]byte, error) {
	...
	service.runner = directTestRunner{}
...
func TestGitCommitUsesOpenedDirectoryAfterVisiblePathSwap(t *testing.T) {
	requireGit(t)
	...
	if err := os.WriteFile(
		filepath.Join(visible, "allowed.txt"),
		[]byte("allowed\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
	}
	runTestGit(t, visible, "add", "allowed.txt")
	if err := os.WriteFile(
	...
	runTestGit(t, outside, "add", "outside.txt")

	resolver := &fakeResolver{
		visible: map[string]string{"project": visible},
		afterOpen: func() {
	...
	response, err := handleGit(
		t,
		resolver,
		mustGitCommitRequest(t, "project", "capability commit", false),
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	subject := strings.TrimSpace(
		runTestGit(t, retained, "log", "--format=%s", "-1"),
	)
	if subject != "capability commit" {
		t.Fatalf("retained repository subject = %q", subject)
	}
	if status := runTestGit(t, outside, "status", "--short"); status !=
		"A  outside.txt\n" {
		t.Fatalf("outside repository status = %q", status)
	}
}
...
func TestGitRejectsDotSegmentRepository(t *testing.T) {
	...
	mustRPCErrorCode(t, response, hostaction.CodeInvalidParams)
}
...
func TestGitRequiresVisibleRepository(t *testing.T) {
	...
	mustRPCErrorCode(t, response, hostaction.CodeProjectNotVisible)
}
