...
// Tests Git capability parameter contracts and schemas.
...
func TestNewGitRequestsEncodeJSONRPCEnvelope(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req, err := hostaction.DecodeRequest(data)
			if err != nil {
				t.Fatal(err)
			}
			if req.JSONRPC != hostaction.JSONRPCVersion || string(req.ID) != "42" || req.Method != tt.wantMethod {
...
func TestDecodeGitResultRoundTrips(t *testing.T) {
	response := hostaction.ResponseOK(json.RawMessage(`1`), Result{Repository: "repo", Stdout: "ok"})
