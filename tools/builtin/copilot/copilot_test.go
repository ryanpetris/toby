...
func TestRegisterContextFilesWritesCopilotFiles(t *testing.T) {
	home := t.TempDir()
	cp, sandbox, holder := newTestCopilot(t, filepath.Join(home, "context"))
...
func newTestCopilot(t *testing.T, contextDir string) (tools.Tool, *fake.Sandbox, *sessionconfig.Holder) {
	t.Helper()
