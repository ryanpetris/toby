...
For each sandbox session the host creates one container, `docker cp`s the sandbox-facing Toby binary into `/toby/bin`, and starts it as `toby sandbox manager`. The manager is proxy-only: its stdin/stdout are a single gRPC connection to the host (the manager is the client, the host the server), and it binds a loopback HTTP listener that tunnels the sandbox's outbound connections to the host. Everything the host does to the sandbox — running the tool, writing files, initializing mounts — goes through the Docker API (`docker exec`, `docker cp`), not an in-sandbox RPC service. Host-written files default to root-owned regular files (`0644`) and directories (`0755`); host-run commands default to the host uid, gid, and supplementary groups.

The host↔sandbox link is a single gRPC connection carried on the container's stdio (the Docker attach stream), so it needs no network port and works the same for local, Docker Desktop, Podman, and remote daemons. The container uses bridge networking; the manager's proxy listener (`127.77.0.1:47600`) stays on the container's own loopback while tools keep outbound network access. Proxied URLs handed to the sandbox point at that listener. The container receives a calculated `HOME`, `TOBY_SANDBOX=1`, and host `TERM` when set; per-command environment is injected into each `docker exec`.
...
The container runtime uses testcontainers-go (the Docker SDK under the hood) to create the container and the moby client for `cp`/`exec`/`attach`/`wait`, rather than the `docker` CLI. The container runs as `--user 0:0` with an init process, so Dockerfile `USER` lines do not prevent the manager and host-run setup work from owning files. The default image is `mcr.microsoft.com/devcontainers/javascript-node:24-bookworm`. A named volume is mounted at `$HOME` (`/toby/home`), and projects live under `/toby/workspace`. The in-container layout (`/toby/home`, `/toby/workspace`, `/toby/bin`, `/toby/context`) is fixed and defined as constants in `container/layout`. The container runs the proxy-only `toby sandbox manager` (its stdio is the gRPC link); the interactive tool runs as a separate `docker exec` with the user's terminal attached to it.
...
A session uses a single container. Each provider volume is mounted at both its final target and an isolated `/toby/mounts/<random>` setup path; after the container starts, mount-init hooks chown the volume at its setup path via a root `docker exec`, without crossing into the binds layered under the final target. Empty volumes seed from the image on first mount.

The `workdir` value is a sandbox-visible path. A leading `~` expands to the sandbox home (`/toby/home`).

The container image is responsible for containing the tools required by the selected Toby tools. Toby mounts the private home and selected projects; it does not install base OS packages into the image.
...
On Linux, the sandbox-facing Toby binary copied into the container comes from the host's own `/proc/self/exe`. macOS release builds embed the matching Linux helper. Local Darwin builds without the release embed tag require `TOBY_LINUX_TOBY` to point at a Linux Toby binary.

Local Linux daemons bind-mount host project paths directly. With a remote daemon, project paths must be reachable by that daemon. The gRPC control link rides the Docker attach stream, so it needs no network port.
