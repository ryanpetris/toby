...
The container runtime uses testcontainers-go (the Docker SDK under the hood) to create the container and the moby client for `cp`/`exec`/`attach`/`wait`, rather than the `docker` CLI. The container runs as `--user 0:0` with an init process, so Dockerfile `USER` lines do not prevent the manager and host-run setup work from owning files. The default image is `mcr.microsoft.com/devcontainers/javascript-node:24-bookworm`. A named volume is mounted at `$HOME` (`/toby/home`), and projects live under `/toby/workspace`. The in-container layout (`/toby/home`, `/toby/workspace`, `/toby/bin`, `/toby/context`) is fixed and defined as constants in `container/layout`. The container's main process is the idle `toby sandbox idle`; the proxy-only `toby sandbox manager` (whose stdio is the gRPC link) and the interactive tool each run as separate `docker exec`s, the latter under a container PTY — attached to the user's terminal when there is one, otherwise streamed to the host stdout (e.g. a systemd journal).
