...
// Interactive attach: wires the host terminal to the running Run container,
// reproducing `docker run -it` so the in-sandbox agent's stdio reaches the
// user's terminal while the control websocket carries only RPCs.
