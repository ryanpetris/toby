...
// Container networking: builds the environment passed to the Setup/Run
// containers and rewrites the control host for the daemon class so the
// in-sandbox manager can reach the host control server.
...
func controlEnv(env environ.Environment, class engine.DaemonClass) map[string]string {
...
func rewriteControlHost(value string, class engine.DaemonClass) string {
	switch class {
	case engine.DaemonDesktop:
		return swapHost(value, "host.docker.internal")
	case engine.DaemonRemote:
...
func controlHostPort(env environ.Environment) int {
...
func networkLabel(class engine.DaemonClass, phase phaseKind) string {
	if phase == phasePrime {
		return ""
	}
	if class == engine.DaemonLocalUnix {
		return "host"
	}
	return "bridge"
}
