...
func TestProxyForwardsRequestAndAppliesConfiguredHeaders(t *testing.T) {
	...
	svc := NewService(upstream.Client())
	id, err := svc.RegisterUpstream(upstream.URL+"/v1", http.Header{"Authorization": []string{"Bearer host-secret"}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestProxyDispatchesInternalHandlerTarget(t *testing.T) {
	svc := NewService(nil)
	id, err := svc.RegisterHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
