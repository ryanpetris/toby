...
- `mcp` entries are added to supported generated tool configs, alongside Toby's built-in MCP server. HTTP-backed MCP entries are exposed to the sandbox as stdio commands such as `toby sandbox mcp docs`; Toby resolves the remote URL and headers on the host side so sandboxed tools do not see those credentials. Generated tool config lives under `/tmp/toby/context` inside the sandbox and does not modify the tools' normal config files.
...
Toby automatically exposes a sandbox-only `toby sandbox mcp` server to supported tools launched through `toby <client>`. For OpenCode, Claude Code, Copilot, and Grok, Toby injects this server through synthetic tool configuration generated under `/tmp/toby/context`. Grok discovers that generated config through a `~/.grok/managed_config.toml` symlink. Codex receives Toby MCP through launch-time `-c` config overrides instead of a generated profile file. The built-in server uses Toby's authenticated WebSocket control connection and provides `git.commit`, `git.fetch`, `git.push`, `git.rebase`, and `git.tag` for repositories already visible in the sandbox.

Configured HTTP-backed MCP servers are exposed as separate stdio MCP entries with their original configured names. For example, an `mcp.docs` entry using `type: remote`, `type: http`, `type: streamable-http`, or `type: sse` is presented to tools as command `toby` with args `sandbox mcp docs`. The sandbox sees a normal stdio MCP and the configured name only; Toby opens the remote HTTP/SSE connection from the host process and applies configured `headers`, `http_headers`, `env_http_headers`, and `bearer_token_env_var` values there. Local/stdio MCP entries are not proxied by this HTTP proxy path.
